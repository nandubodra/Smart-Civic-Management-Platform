...
drop policy if exists "admins and workers can update complaints" on complaints;
...
drop policy if exists "workers and admins can view workers" on workers;
...
-- AI is optional: set OPENAI_API_KEY server-side for image classification. Never expose it as NEXT_PUBLIC_*. 
-- Create complaint-photos and add authenticated Storage policies before enabling uploads.
